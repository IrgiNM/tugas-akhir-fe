...
const HeadTopReports = ({pick, click, click2}: headButtonJenisLogType) => {
  const [jenisLog, setJenisLog] = useState('HTTP Request')
  const {dataLogDataset} = DataLogDatasetFunction()

  return (
    <div className='w-full bg-[#10101b] flex flex-col py-3 px-5 gap-3 rounded-md'>
        <div className='flex flex-row gap-2 items-center'>
            <p className='font-bold text-[15px]'>Security Dashboard Report</p>
            <p className=' p-2 px-4 border-1 border-gray-700 rounded-lg'>Monitor top blocked traffic, attacks, malware, protocols, and security activity from WatchGuard Firebox reports.</p>
        </div>
        <div className='flex flex-row gap-2 items-center'>
            {/* <div className='flex flex-row gap-2 items-center'>
