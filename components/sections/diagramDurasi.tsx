...
const DiagramDurasi = () => {
  const {dataDurasi, dataCount, dataTenY, dataNerworkTraffic} = dataNetworkTrafficFunction()
  ...
  return (
    <div className='w-full flex flex-col border-1 border-white rounded-md p-5'>
        <p className='text-[20px] font-bold'>Durasi</p>
        <p className='mb-7'>Statistik banyak nya Durasi yang ada ( satuan detik s )</p>
        {dataNerworkTraffic.length === 0 ? (
            ...
            <div className='w-full h-full rounded-md flex flex-col pr-2 items-start justify-center gap-3'>
                {dataDurasi.map((item,index)=>{
                    const count = Math.round(330/dataCount)
                    return (
                        <div className='w-full flex flex-row'>
                            <p className={`font-bold text-[12px] w-[50px] ${datacolorText[index]}`}>{(parseFloat(item.name)).toFixed(4)}</p>
